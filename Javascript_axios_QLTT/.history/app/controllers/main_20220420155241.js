...
function update(id) {
    ...
    document.getElementById("header").innerHTML = title;
    var button = "<button class='btn btn-success' onclick='addUser()'>Cập nhật</button>";
    document.getElementById("btn-footer").innerHTML = button;
    ...
    services.getDataAPI(id)
        .then(function(result) {
            ...
            getEle("HoTen").value = user.taiKhoan;
            getEle("MatKhau").value = user.taiKhoan;
            getEle("Email").value = user.taiKhoan;
            getEle("HinhAnh").value = user.taiKhoan;
            getEle("loaiNguoiDung").value = user.taiKhoan;
            getEle("loaiNgonNgu").value = user.taiKhoan;
            getEle("MoTa").value = user.taiKhoan;
        })
        .catch(function(error) {
            console.log(error);
        })
